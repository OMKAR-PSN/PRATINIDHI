        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="font-heading text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
              <Users className="w-6 h-6 text-blue-600" /> {t('recv_title', 'My Receivers')}
            </h1>
            <p className="text-gray-500 text-sm mt-1">{t('recv_sub', 'Manage your private broadcast list')}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="csvUpload" className="px-4 py-2.5 rounded-xl glass-card text-gray-700 font-bold text-sm flex items-center gap-2 cursor-pointer hover:bg-white transition-all btn-press shadow-sm border border-gray-200/50">
              <Upload className="w-4 h-4 text-emerald-500" /> {t('recv_upload', 'Upload CSV')}
            </label>
          ...
          <div className="lg:col-span-1">
            <div className="glass-card rounded-2xl p-6 shadow-sm">
              <h3 className="font-heading font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <UserPlus className="w-5 h-5 text-saffron-500" /> {t('recv_add_contact', 'Add New Contact')}
              </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1 text-sm">
                  <label className="font-semibold text-gray-700">{t('recv_full_name', 'Full Name *')}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1 text-sm">
                    <label className="font-semibold text-gray-700 flex items-center gap-1"><Phone className="w-3 h-3"/> {t('recv_phone', 'Phone')}</label>
                  ...
                  <div className="space-y-1 text-sm">
                    <label className="font-semibold text-gray-700 flex items-center gap-1"><Mail className="w-3 h-3"/> {t('recv_email', 'Email')}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1 text-sm">
                    <label className="font-semibold text-gray-700 flex items-center gap-1"><Globe className="w-3 h-3"/> {t('recv_language', 'Language')}</label>
                  ...
                  <div className="space-y-1 text-sm">
                    <label className="font-semibold text-gray-700">{t('recv_type', 'Type')}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1 text-sm">
                    <label className="font-semibold text-gray-700 flex items-center gap-1"><MapPin className="w-3 h-3"/> {t('recv_state', 'State')}</label>
                  ...
                  <div className="space-y-1 text-sm">
                    <label className="font-semibold text-gray-700">{t('recv_district', 'District')}</label>
                ...
                <button type="submit" disabled={adding} className="w-full mt-4 py-3 rounded-xl gradient-primary text-white font-bold text-sm tracking-wide flex items-center justify-center gap-2 btn-press transition-all hover:shadow-lg disabled:opacity-50">
                  {adding ? <Loader2 className="w-4 h-4 animate-spin" /> : t('recv_save_btn', 'Save Contact')}
                </button>
              </form>
            </div>
          </div>
            ...
            <div className="glass-card rounded-2xl p-6 shadow-sm overflow-hidden h-full">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-heading font-semibold text-gray-900 flex items-center gap-2">
                  <CheckSquare className="w-5 h-5 text-emerald-500" /> {t('recv_roster', 'Contact Roster')}
                </h3>
                <span className="text-xs font-bold px-2.5 py-1 bg-gray-100 rounded-lg text-gray-600 border border-gray-200">
                  {t('recv_total', 'Total:')} {receivers.length}
                </span>
              </div>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-48 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                  ...
                  <p className="text-sm font-semibold text-gray-500">{t('recv_empty', 'No receivers added yet.')}</p>
                  <p className="text-xs text-gray-400 mt-1">{t('recv_empty_sub', 'Use the form to build your list.')}</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    <thead>
                      <tr className="border-b border-gray-200 text-gray-500 font-semibold uppercase tracking-wider text-[11px]">
                        <th className="pb-3 px-2">{t('recv_name_col', 'Name')}</th>
                        <th className="pb-3 px-2">{t('recv_contact_col', 'Contact')}</th>
                        <th className="pb-3 px-2">{t('recv_lang_col', 'Language')}</th>
                        <th className="pb-3 px-2">{t('recv_loc_col', 'Location')}</th>
                        <th className="pb-3 px-2">{t('recv_tags_col', 'Tags')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {receivers.map(r => (
