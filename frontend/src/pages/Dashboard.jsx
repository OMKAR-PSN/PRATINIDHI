    ...
    <div className="min-h-screen gradient-subtle tech-grid">
      ...
      <div className="ml-64 p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="font-heading text-2xl font-bold text-gray-900 tracking-tight">Dashboard</h1>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <StatCard icon={Video} title="Total Avatars Generated" value="1,247" change="+12%" changeType="up" color="blue" />
          <StatCard icon={Languages} title="Languages Used" value="18" change="+3" changeType="up" color="saffron" />
          <StatCard icon={Users} title="Citizens Reached" value="2.4M" change="+8.2%" changeType="up" color="green" />
          <StatCard icon={MessageSquare} title="Messages Delivered" value="3,891" change="+15%" changeType="up" color="purple" />
        </div>
        ...
        <div className="mt-8 glass-card rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100/50 flex items-center justify-between">
            <h2 className="font-heading font-semibold text-gray-900">Recent Announcements</h2>
            <Link to="/preview" className="text-sm text-blue-500 font-medium hover:underline flex items-center gap-1">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="badge-lang text-[10px] font-bold px-2 py-0.5 rounded-full">{item.lang}</span>
                      <span className="text-xs text-gray-300">•</span>
                      <span className="flex items-center gap-1 text-xs text-gray-400"><Clock className="w-3 h-3" /> {item.date}</span>
                    </div>
                  </div>
                </div>
                <span className={`text-[11px] font-semibold px-3 py-1 rounded-full ${
                  item.status === 'Delivered' ? 'bg-emerald-50/80 text-emerald-600' : 'bg-saffron-50/80 text-saffron-600'
                }`}>{item.status}</span>
              </div>
            ))}
          </div>
        ...
        <div className="mt-8 grid sm:grid-cols-3 gap-5">
          {[
            { to: '/create', icon: Video, color: 'blue', title: 'Create New Avatar', desc: 'Generate a multilingual avatar announcement' },
            { to: '/analytics', icon: Users, color: 'saffron', title: 'View Analytics', desc: 'Track citizen engagement & reach' },
            { to: '/ask-avatar', icon: MessageSquare, color: 'emerald', title: 'Ask Avatar Q&A', desc: 'AI-powered citizen query system' },
