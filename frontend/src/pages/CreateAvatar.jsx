...
export default function CreateAvatar() {
  ...
  // Step 4: Pipeline
  const [language, setLanguage] = useState(() => localStorage.getItem('draftLanguage') || 'hi')
  ...
  const handleGenerate = async () => {
    ...
    if (!token) {
      ...
      localStorage.setItem('draftLanguage', language)
    ...
    // Save the user's ACTUAL message + settings for the preview page
    const langLabel = languages.find(l => l.code === language)?.label || language
    localStorage.setItem('generatedMessage', message)
    localStorage.setItem('generatedLanguage', language)
    localStorage.setItem('generatedLanguageLabel', langLabel)
    localStorage.setItem('generatedAvatar', selectedAvatar || '')
    ...
    const interval = setInterval(() => {
      ...
      if (currentStep >= 5) {
        ...
        setTimeout(() => window.location.href = '/preview/generated', 1000)
      }
    }, 1500)
  }
