...
export default function AskAvatar() {
  ...
  const handleTranslate = async (msgIdx) => {
    ...
    const srcLang = AVATAR_LANG[selectedAvatar] || 'hi'
    ...
    try {
      ...
      setHistory(prev => {
        const updated = [...prev]
        updated[msgIdx] = { ...updated[msgIdx], translatedText: data.translated_text }
        return updated
      })
    } catch (err) {
  ...
  const handleAsk = async (q) => {
    ...
    try {
      ...
      const res = await fetch(`/ask_text/${selectedAvatar}`, {
        method: 'POST',
        body: formData,
      });
      const data = await res.json();
      ...
      const errMsg = {
        type: 'ai',
        text: 'Sorry, I am currently offline or cannot process your request.',
        time: new Date().toLocaleTimeString(),
        videoUrl: null,
      }
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 flex justify-between items-center">
              ...
              <select 
                ...
                className="px-4 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 bg-white"
              >
                <option value="arjun">Arjun (Hindi)</option>
                <option value="priya">Priya (Marathi)</option>
                <option value="murugan">Murugan (Tamil)</option>
                <option value="asha">Asha (Bengali)</option>
                <option value="bharat">Bharat (English)</option>
              </select>
            </div>
          </div>
        </div>
              ...
              {history.map((msg, i) => (
                <div key={i} className={`flex gap-3 animate-fade-in-up ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                    ...
                    {msg.translatedText && (
                      <p className="mt-2 pt-2 border-t border-gray-200/50 text-xs text-gray-500 italic">
                        🌐 {msg.translatedText}
                      </p>
                    )}
                    <p className={`text-[10px] mt-1.5 ${msg.type === 'user' ? 'text-white/50' : 'text-gray-400'}`}>{msg.time}</p>
                    {msg.type === 'ai' && !msg.translatedText && (
                      <button
                        onClick={() => handleTranslate(i)}
                        disabled={translating[i]}
                        className="mt-1.5 flex items-center gap-1 text-[11px] font-medium text-blue-500 opacity-60 hover:opacity-100 transition-opacity disabled:opacity-30"
                      >
                        {translating[i] ? (
                          <><span className="w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" /> Translating...</>
                        ) : (
                          <>🌐 Translate to {AVATAR_LANG[selectedAvatar] === 'en' ? 'Hindi' : 'English'}</>
                        )}
                      </button>
                    )}
                  </div>
