...
const AVATAR_PROFILES = {
  arjun: { name: 'Arjun', emoji: '👳', color: '#1D9E75' },
  priya: { name: 'Priya', emoji: '👩‍💼', color: '#534AB7' },
  murugan: { name: 'Murugan', emoji: '👨‍🏫', color: '#D85A30' },
  asha: { name: 'Asha', emoji: '👩‍⚕️', color: '#BA7517' },
  bharat: { name: 'Bharat', emoji: '👨', color: '#1D4ED8' },
}
      ...
      .then(data => {
        if (data.broadcasts) {
          const formatted = data.broadcasts.map(b => ({
            ...
            original_text: b.message,
            language: b.avatar_id === 'arjun' ? 'hi' : b.avatar_id === 'priya' ? 'mr' : b.avatar_id === 'murugan' ? 'ta' : b.avatar_id === 'bharat' ? 'en' : 'bn',
            status: b.status,
            regions: ['All Regions'],
            reach: 0,
            views: 0,
          ...
          setMessages([...formatted, ...initialDemoMessages]);
        }
      })
      .catch(console.error);
  }, []);
                ...
                {messages.map((m) => (
                  <tr key={m.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      {m.avatar_id && AVATAR_PROFILES[m.avatar_id] && (
                        <div className="flex items-center gap-2 mb-2">
                          <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs" style={{backgroundColor: AVATAR_PROFILES[m.avatar_id].color + '20'}}>
                            {AVATAR_PROFILES[m.avatar_id].emoji}
                          </div>
                          <span className="text-xs font-bold" style={{color: AVATAR_PROFILES[m.avatar_id].color}}>
                            {AVATAR_PROFILES[m.avatar_id].name} - AI Spokesperson
                          </span>
                        </div>
                      )}
